        ...
        <div className="flex-1 space-y-2 text-sm">
            ...
            {store.user && (
              <div className="flex items-center gap-2 text-sm ">
                  <Image width={36} height={36} src={store.user.image || '/default-avatar.png'} alt={store.user.name || 'User'} className="w-9 h-9 rounded-full" />
                  <div>
                      <p className="text-slate-600 font-medium">{store.user.name || 'Unknown User'}</p>
                      <p className="text-slate-400">{store.user.email || 'No email provided'}</p>
                  </div>
              </div>
            )}
        </div>
